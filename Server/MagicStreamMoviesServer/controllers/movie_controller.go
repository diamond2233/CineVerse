...
import (
	...
	"github.com/diamond22233/MagicStreamMovies/Server/MagicStreamMoviesServer/models"
...
func AdminReviewUpdate(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if role != "ADMIN" {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "user must be an admin"})
			return
		}
...
func GetRecommendedMovies(client *mongo.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		findOptions.SetSort(bson.D{{Key: "ranking.ranking_value", Value: 1}}) //value 1 for ascending order
